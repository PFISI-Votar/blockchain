...
const startHardhatNode = () => {
  ...
  const child = spawn('npx', ['hardhat', 'node'], spawnOpts({
    stdio: ['ignore', 'pipe', 'pipe'],
  }));
  child.stdout.on('data', (chunk) => process.stdout.write(`[hardhat] ${chunk}`));
  child.stderr.on('data', (chunk) => process.stderr.write(`[hardhat] ${chunk}`));
  return child;
};
  ...
  try {
    ...
    hardhatProcess.on('exit', (code) => {
      if (!isShuttingDown && code !== 0 && code !== null) {
        console.error(`[blockchain] Hardhat node terminó inesperadamente (${code})`);
        process.exit(code);
      }
    });
